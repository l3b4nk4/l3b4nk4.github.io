document.addEventListener("DOMContentLoaded", function () {
  ...
  const searchInput = searchInputWrapper
  ...
  if (!searchInput || !searchResults || !searchStats) return;
  ...
  if (siteSearch) {
    siteSearch.addEventListener("click", (event) => {
      if (event.target === siteSearch) closeSearch();
    });
  }
  document.addEventListener("click", (event) => {
    if (!popup || !popup.classList.contains("show")) return;
    if (navSearchBtn && navSearchBtn.contains(event.target)) return;
